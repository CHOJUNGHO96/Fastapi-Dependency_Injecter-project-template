    ...
    def filter_stream(self, stream):
        ...
        while not stream.eos:
            ...
            if token.test("variable_begin"):
                ...
                # don't bind twice
                if not last_token.test("name") or not last_token.value in (
                    "bind",
                    "any",
                    "sqlsafe",
                ):
...
# pandas는 pyformat로 포맷설정하고 sqlalchemy는 named로 포맷설정 해야한다.
JINJA_SQL = {
    "pyformat": JinjaSql(param_style="pyformat"),
    "named": JinjaSql(param_style="named"),
}
